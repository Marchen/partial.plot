...
* [Visualizing slighly complicated model](#basic_usage2)
...
By recent active development of R and it's community, now ecologists have
several kind of models to test and explain results and predict 
...
If the command above couldn't work, try this one.
...
## <a id = "quickstart">Quick start (for R wizards)</a>
...
# <a id = "basic_usage2">Visualizing slighly complicated models</a>
...
from Quebec and Mississippi in several CO<sub>2</sub> concentraitons
...
By setting 'extrapolate' to 'TRUE', predicted partial relationshiops are drawn
for range of the expranatory variable for all groups.
```{R}
info <- partial.plot(
