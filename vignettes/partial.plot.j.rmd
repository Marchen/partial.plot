...
# データの傾向を見てみる。
plot(
	uptake ~ conc, data = CO2, col = color.ramp(CO2$Type),
	pch = color.ramp(CO2$Treatment, c(chilled = 16, nonchilled = 17)),
	ylim = c(0, 50)
)
legend(
	"bottomright",
	...
	col = rep(color.ramp(CO2$Type, unique.pal = TRUE), each = 2),
	pch = c(16, 17, 16, 17)
)

```

上の図を見るとなんとなく、CO<sub>2</sub>濃度とCO<sub>2</sub>吸収の関係は線形ではなさそうです。
そして、


```{R}
model <- glm(uptake ~ (conc + I(conc^2))  +(Type * Treatment), data = CO2)
info <- partial.plot(model, c("conc", "Treatment", "Type"), data = CO2,sep=" -- ")
pp.legend(info, "topright")
```
...
* 2016.12.31: 0.0.5
	* グループ変数の区切り文字をコントロールするsepオプションをpartial.plot()関数に追加。
* 2016.07.23: 0.0.4
